...
with driver.session() as session:
    # Verify Process node
    result = session.run(
        'MATCH (p:Process {process_id: "boston_resident_parking_permit"}) RETURN p.name, p.source_url, p.confidence'
    )
    ...
    step_count = session.run(
        'MATCH (s:Step) WHERE s.process_id = "boston_resident_parking_permit" RETURN count(s) as count'
    ).single()["count"]
    print(f"\n✓ Step Nodes: {step_count} (expected: 3)")

    # Count Requirement nodes
    req_count = session.run(
        'MATCH (r:Requirement) WHERE r.applies_to_process = "boston_resident_parking_permit" RETURN count(r) as count'
    ).single()["count"]
    print(f"✓ Requirement Nodes: {req_count} (expected: 4)")

    # Count DocumentType nodes
    doc_count = session.run(
        'MATCH (dt:DocumentType) WHERE dt.doc_type_id STARTS WITH "proof." RETURN count(dt) as count'
    ).single()["count"]
    ...
    print(
        f"  With source_url: {citations['with_source']} (100%: {citations['with_source'] == citations['total']})"
    )
    print(
        f"  With last_verified: {citations['with_verified']} (100%: {citations['with_verified'] == citations['total']})"
    )
    print(
        f"  With confidence: {citations['with_confidence']} (100%: {citations['with_confidence'] == citations['total']})"
    )
    ...
    for rel_type, description, expected in rels:
        count = session.run(f"MATCH ()-[r:{rel_type}]->() RETURN count(r) as count").single()[
            "count"
        ]
